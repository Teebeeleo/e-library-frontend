    ...
    <header className="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm">
      ...
      <div className="flex items-center gap-4">
        <div className="hidden sm:block text-right">
          <p className="text-gray-900 text-sm font-semibold leading-tight">{user.name || user.email}</p>
          <p className="text-gray-400 text-xs">{user.level ? `Level: ${user.level}` : user.role}</p>
        </div>
        <button
          onClick={logout}
